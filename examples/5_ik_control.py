...
from beingbeyond_d1_edu_sdk.pin_kinematics import (
    D1Kinematics,
    D1KinematicsConfig,
    IKOptions,
)
...
def main():
    ...
    robot = HeadArmRobot(urdf_path=urdf, dev=dev, baudrate=baudrate)
    try:
        joint_names = robot.joint_names
        n_joints = len(joint_names)
        print("[Info] Joint order:", joint_names)
        print("[Info] Device:", dev)
        print("[Info] URDF:", urdf)
        ...
        for outer in range(max_outer_iters):
            ...
            if not np.all(np.isfinite(q_head_sol)) or not np.all(np.isfinite(q_arm_sol)):
                print("[IK] Invalid IK solution (NaN/Inf), stopping.")
                break
